// fa-layout.js-> fa stands for "Farsi" (Persian), but the layout is based on the Arabic keyboard, which is commonly used in Persian typing. This script provides functions to convert text between English and Arabic layouts, as well as checks for the presence of Persian and English characters.
...
const faLayout = {
    ...
    fromEn: function (text, includeNumbers) {
        return text.split('').map(char => {
            // Skip digits unless explicitly requested (manual Ctrl+Q)
            if (!includeNumbers && /\d/.test(char)) return char;
    ...
    // Note: digits (0-9) excluded — numbers are not keyboard layout mistakes
